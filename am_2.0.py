#!/usr/bin/env python
#
# am2 -- M. Templeton, September 20, 2017
#
#----------------------------------------------------------------------------
...
#       match_aflist.append(dict_frame.Affcode[p>crit].tolist())
        match_aflist.append(dict_frame.Affcode[p==p.max()].tolist())
...
def final_match(match_frame):
    ...
    affil_canonical=match_frame['Affil'].tolist()
    test_answers=match_frame['Affcodes'].tolist()
    (children,parents,canonical)=parents_children()
    
    mc=open('matched_child.txt','w')
    mp=open('matched_parent.txt','w')
    mpc=open('matched_parents_child.txt','w')
    mw=open('wrong_match.txt','w')
    for r,a,t in zip(right_answers,affil_canonical,test_answers):
        t=t[0]
        if r == t:
            mc.write("%s\t%s\t%s\n"%(r,a,t))
        else:
