...
from adsmsg.augmentrecord import AugmentAffiliationResponseRecord, AugmentAffiliationResponseRecordList
import pandas as pd
...
def to_dataframe(message):
    ...
    if message_type == 'affil_request':
        ...
        raise ValueError('to_dataframe does not handle message of type %s' % type(message))


def to_response(request, answer, answer_id):
    """generate response protobuf to send to master pipeline"""
    message_type = app.get_msg_type(request)
    if message_type == 'affil_request':
        ...
        raise ValueError('to_response does not handle message of type %s' % type(request))
